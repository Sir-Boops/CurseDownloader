...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Use Forge Version: " + manifest.getJSONObject("minecraft").getJSONArray("modLoaders").getJSONObject(0).getString("id"));
        System.out.println("!!!!!!!!!!!!!!!!!!!!");

        File oldPath = new File(Main.fullPath);
        File newPath = new File(new File(".").getCanonicalPath() + File.separator + manifest.getString("name") + "-" + manifest.getString("version") + File.separator);

        if (args.length == 2) {
            if (args[1].equalsIgnoreCase("--mcboop")) {
                newPath = new File(System.getProperty("user.home") + File.separator + ".mcboop" + File.separator + "profiles" + File.separator + manifest.getString("name") + "-" + manifest.getString("version") + File.separator);
            }
        }
        ...
        if (args.length == 2) {
            if (args[1].equalsIgnoreCase("--mcboop")) {
                JSONObject profile = new JSONObject().put("mcVersion", manifest.getJSONObject("minecraft").getString("version")).put("forgeVersion", manifest.getJSONObject("minecraft").getJSONArray("modLoaders").getJSONObject(0).getString("id"));
